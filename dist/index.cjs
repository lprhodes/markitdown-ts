...
var CHARS, BLANK, BACKSLASH, ALN, BRK, FUNC_PLACE, CHR, CHR_BO, T, FUNC, CHR_DEFAULT, POS, POS_DEFAULT, SUB, SUP, F, F_DEFAULT, D, D_DEFAULT, RAD, RAD_DEFAULT, ARR, LIM_FUNC, LIM_TO, LIM_UPP, M;
var init_latex_dict = __esm({
  "src/utils/docx-math/latex-dict.ts"() {
    "use strict";
...
var omml_to_latex_exports = {};
__export(omml_to_latex_exports, {
  ommlToLatex: () => ommlToLatex,
  processOmmlInXml: () => processOmmlInXml
});
...
function parseOmml(xml) {
  const parser = new import_fast_xml_parser.XMLParser(OMML_PARSER_OPTIONS);
...
var import_fast_xml_parser, M_PREFIX, OMML_PARSER_OPTIONS, PR_VAL_TAGS, PR_TAG_METHODS, DIRECT_TAGS, TAG_METHODS;
var init_omml_to_latex = __esm({
  "src/utils/docx-math/omml-to-latex.ts"() {
    "use strict";
    ...
    PR_VAL_TAGS = /* @__PURE__ */ new Set(["chr", "pos", "begChr", "endChr", "type"]);
    PR_TAG_METHODS = {
      brk: "brk",
      chr: "common",
      pos: "common",
...
`;
  ...
  async preProcessDocx(buffer, JSZip) {
    ...
    for (const path of xmlFiles) {
      ...
      const { processOmmlInXml: processOmmlInXml2 } = await Promise.resolve().then(() => (init_omml_to_latex(), omml_to_latex_exports));
      const newXml = processOmmlInXml2(xml);
      if (newXml !== xml) {
...
var RssConverter = class {
  ...
  async convert(input, _info, _opts) {
    ...
    const parser = new import_fast_xml_parser2.XMLParser(SAFE_XML_OPTIONS);
    let doc;
    try {
...
var PptxConverter = class {
  htmlConverter = new HtmlConverter();
  parser = new import_fast_xml_parser3.XMLParser(SAFE_XML_OPTIONS);
  accepts(info) {
    const ext = (info.extension ?? "").toLowerCase();
    const mime = (info.mimetype ?? "").toLowerCase();
...
var import_fast_xml_parser4 = require("fast-xml-parser");
init_xml_utils();
...
var EpubConverter = class {
  htmlConverter = new HtmlConverter();
  parser = new import_fast_xml_parser4.XMLParser(SAFE_XML_OPTIONS);
  accepts(info) {
    const ext = (info.extension ?? "").toLowerCase();
    const mime = (info.mimetype ?? "").toLowerCase();
    ...
    if (injected?.pdfjsLib) {
      ...
      try {
        ...
        throw new MissingDependencyError(
          "pdfjs-dist",
          "pnpm add pdfjs-dist, or pass nodeServices.pdfjsLib to MarkItDown for bundled/serverless environments. Underlying resolution error: " + (err instanceof Error ? `${err.name}: ${err.message}` : String(err))
        );
      }
    }
      ...
      try {
        ...
        try {
          ...
          pdfjsBuildDir = dirname(
            fileURLToPath(import_meta.resolve("pdfjs-dist/legacy/build/pdf.mjs"))
          );
        } catch {
          const { createRequire } = await import("module");
          const anchor = typeof import_meta.url === "string" && import_meta.url.startsWith("file:") ? import_meta.url : typeof __filename === "string" ? __filename : void 0;
          if (anchor) {
